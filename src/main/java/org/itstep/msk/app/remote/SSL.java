...
public class SSL {
    ...
    public static String PRIVATE_KEY_FILE = "/etc/wiy/ssh/private_key";//server
     static final String PUBLIC_KEY_FILE = "/etc/wiy/ssh/public_key";//server
    public static byte[] PUBLIC_KEY;//client from sql
    SSL(){

    }
    SSL(String pub_key){
        //PRIVATE_KEY_FILE=pk;
        PUBLIC_KEY=hex2Byte(pub_key);
    ...
    public static void setPublicKey(String pub_key){
        PUBLIC_KEY=hex2Byte(pub_key);
    }
    ...
    private static byte[] fileToKey(String file) throws IOException {
     ...
     static PublicKey restorePublic() throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
        KeyFactory keyFactory = KeyFactory.getInstance(ALGORITHM);
        EncodedKeySpec publicKeySpec = new X509EncodedKeySpec(PUBLIC_KEY);
        return keyFactory.generatePublic(publicKeySpec);
    }

    static PrivateKey restorePrivate() throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
        KeyFactory keyFactory = KeyFactory.getInstance(ALGORITHM);
